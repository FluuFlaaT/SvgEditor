...
// Convert a QColor to the engine's Color struct.
static inline Color colorFromQColor(const QColor& c)
{
    return Color{c.red(), c.green(), c.blue(), c.alpha()};
}

// Apply common stroke + fill + opacity from Qt pen/brush to an SVG element.
static inline void applySvgStyle(SvgElement* el,
                                  const QPen& pen,
                                  const QBrush& brush,
                                  qreal opacity)
{
    el->setStrokeColor(colorFromQColor(pen.color()));
    el->setFillColor(colorFromQColor(brush.color()));
    el->setStrokeWidth(pen.width());
    el->setOpacity(static_cast<double>(opacity));
}

// Overload for stroke-only elements (lines, polylines – no fill).
static inline void applySvgStyle(SvgElement* el,
                                  const QPen& pen,
                                  qreal opacity)
{
...
void CanvasArea::makeItemInteractive(QGraphicsItem* item)
{
...
QGraphicsEllipseItem* CanvasArea::createEllipse(const QPointF& startPoint, const QPointF& endPoint)
{
    ...
    makeItemInteractive(ellipseItem);
    return ellipseItem;
}
...
QGraphicsPolygonItem* CanvasArea::createRegularPolygon(const QPointF& center, qreal radius, int sides)
{
    ...
    makeItemInteractive(polygonItem);
...
void CanvasArea::addShapeToDocument(QGraphicsItem* item)
{
    ...
    if (QGraphicsLineItem* lineItem = dynamic_cast<QGraphicsLineItem*>(item)) {
        ...
        auto svgLine = std::make_unique<SvgLine>(Point{line.x1(), line.y1()},
                                                  Point{line.x2(), line.y2()});
        applySvgStyle(svgLine.get(), lineItem->pen(), item->opacity());
        ...
        auto svgRect = std::make_unique<SvgRectangle>(Point{rect.x(), rect.y()},
                                                       rect.width(), rect.height());
        applySvgStyle(svgRect.get(), rectItem->pen(), rectItem->brush(), item->opacity());
        ...
        if (m_currentShapeType == ShapeType::Pentagon) {
            ...
            applySvgStyle(svgPentagon.get(), pen, brush, item->opacity());
            ...
            applySvgStyle(svgHexagon.get(), pen, brush, item->opacity());
            ...
            auto svgStar = std::make_unique<SvgStar>(Point{center.x(), center.y()},
                                                      outerRadius, innerRadius);
            applySvgStyle(svgStar.get(), pen, brush, item->opacity());
            doc->addElement(std::move(svgStar));
        } else {
            auto svgPolygon = std::make_unique<SvgPolygon>(points);
            applySvgStyle(svgPolygon.get(), pen, brush, item->opacity());
        ...
        std::vector<Point> points;
        points.reserve(static_cast<size_t>(path.elementCount()));
        for (int i = 0; i < path.elementCount(); ++i) {
            QPainterPath::Element element = path.elementAt(i);
            points.push_back({element.x, element.y});
        }
        auto svgPolyline = std::make_unique<SvgPolyline>(points);
        applySvgStyle(svgPolyline.get(), pathItem->pen(), item->opacity());
        ...
        auto svgText = std::make_unique<SvgText>(Point{pos.x(), pos.y()},
                                                  textItem->toPlainString().toStdString());
        ...
        svgText->setFillColor(colorFromQColor(textItem->defaultTextColor()));
    ...
    // For backward compatibility, also handle QGraphicsSimpleTextItem
        ...
        svgText->setFillColor(colorFromQColor(textItem->brush().color()));
        svgText->setOpacity(item->opacity());
        doc->addElement(std::move(svgText));
        doc->m_graphicsItems.push_back(item);
